...
def combine_flux_channels(
    ...
    """
    all_channels = list(flux_channels.keys())
    result_channels = {}

    for band in set(channel.band for channel in all_channels):
        band_channels = {
            channel: flux
        ...
        raw_merged = pd.concat([flux_channels[channel] for channel in raw_band_channels], axis=1)
        clean_flux_channels = [flux_channels[channel] for channel in clean_band_channels]
